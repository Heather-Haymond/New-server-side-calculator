...
function getCalcs() {
  axios({
    ...
    //⬅ THEN when server gets the data from the sever, we respond with the data that was requested. Here it is the calculations array of objects that are the calculation attributes.
    ...
    for (let calc of calcHerStory) {
      ...
      let calcListItem = ` 
            <li>${calc.num1} ${calc.operator} ${calc.num2} = ${calc.result}</>
